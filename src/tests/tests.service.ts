...
export class TestsService {
  ...
  public async updateTestStatus(testId: string, status: TestStatus) {
    ...
    try {
      ...
      // Note: AI insights are generated by the calling service to avoid circular dependency
      // The /insights endpoint handles AI insights generation when called by N8N

      return result;
    } catch (error) {
      this.logger.error(`Failed to update test status for ${testId}:`, error);
      throw error;
    }
  }
